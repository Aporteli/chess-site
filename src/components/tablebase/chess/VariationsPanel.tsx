  ...
  return (
    <div className="flex min-h-0 max-h-56 flex-col gap-2 overflow-hidden rounded-xl bg-surface p-3 shadow-[var(--shadow-border)] xl:max-h-[32%]">
      ...
      <div
        ...
        style={{
          scrollbarWidth: "thin",
          scrollbarColor: "var(--color-hint) var(--color-surface)",
        }}
      >
        <style>
          {`
            /* For Webkit browsers */
            .variations-scrollbar::-webkit-scrollbar {
              width: 7px;
              background: var(--color-surface);
            }
            .variations-scrollbar::-webkit-scrollbar-thumb {
              background: var(--color-hint);
              border-radius: 6px;
              min-height: 36px;
            }
            .variations-scrollbar::-webkit-scrollbar-thumb:hover {
              background: var(--color-accent);
            }
          `}
        </style>
        <div className="variations-scrollbar h-full">
          {typed.length === 0 ? (
