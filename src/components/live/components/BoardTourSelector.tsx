...
export function BoardTourSelector({
  broadcasts,
  ...
  return (
    <div className="relative min-w-0 max-w-full">
      <button
        ...
        <span className="min-w-0 max-w-[5.5rem] truncate font-semibold text-white sm:max-w-[9rem] lg:max-w-[14rem] xl:max-w-[240px]">
          {broadcasts[selectedBroadcastIndex]?.tour?.name ?? 'Select tournament'}
        </span>
        ...
        <div className="absolute left-0 top-full z-50 w-[min(18rem,calc(100vw-1.5rem))] pt-1 animate-in fade-in-0 slide-in-from-top-1 duration-100">
          <div className="flex flex-col overflow-hidden rounded-lg border border-[#383838] bg-[#1E1E1E] shadow-2xl">
            ...
            <div className="thin-scrollbar max-h-64 overflow-y-auto p-1">
              {broadcasts.map((broadcast: BroadcastItem, index: number) => {
                const active = broadcast.tour.id === selectedBroadcast?.tour.id;
                ...
                return (
                  <div
                    ...
                    onClick={() => onSelectBroadcast(index)}
                    onContextMenu={(e) => e.preventDefault()}>
                    {/* 👇 აქ უნდა იყოს კონტენტი */}
                    <span className="truncate">{broadcast.tour.name}</span>
