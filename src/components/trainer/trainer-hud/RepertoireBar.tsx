    ...
    <div className="hidden rounded-xl border border-border-subtle bg-bg-surface p-3">
      ...
      {creating && (
        <form
          ...
          />
          <button type="submit" className="rounded-md bg-accent-gold px-2 text-[11px] font-semibold text-[#241a10]">
            Add
          </button>
        </form>
      )}
      ...
      <div className="flex flex-col gap-1">
        {t.store.repertoires.map((rep) => (
          <button
            ...
            <span className="ml-2 shrink-0 font-mono text-[10px] uppercase text-text-muted">{rep.side[0]}</span>
          </button>
        ))}
      </div>
          ...
          {t.repertoire.chapters.map((ch) => (
            <button
              ...
              <span className="min-w-0">
                ...
                {ch.eco && <span className="font-mono text-[10px] text-accent-gold/70">{ch.eco}</span>}
              </span>
              <span className="ml-2 font-mono text-[10px] text-text-muted">{nodeCount(ch)}</span>
            </button>
          ))}
        </div>
      </div>
