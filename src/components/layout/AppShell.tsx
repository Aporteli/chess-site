...
export function AppShell({ activeKey, children }: AppShellProps) {
  ...
  useEffect(() => {
    const narrow = window.matchMedia('(min-width: 1024px) and (max-width: 1279px)');
  ...
  return (
    <div
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        ...
        <main className="flex min-h-0 w-full flex-1 flex-col overflow-y-auto overflow-x-hidden">{children}</main>
      </div>
    </div>
  );
}
