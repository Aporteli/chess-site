      ...
      <div className="mb-1.5 flex shrink-0 items-center justify-between gap-2">
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="font-mono text-sm font-semibold text-accent-gold-bright">
            {engine.evaluation !== null ? (engine.evaluation > 0 ? `+${engine.evaluation}` : engine.evaluation) : "0.00"}
          </span>
      ...
      <div className="flex flex-1 flex-wrap content-start gap-1 overflow-y-auto pr-1 font-mono text-xs">
        {history.length === 0 ? (
          ...
          history.map((san, index) => (
            <span key={`${san}-${index}`} className="rounded border border-border-subtle bg-bg-elevated px-1.5 py-0.5 text-xs text-text-secondary">
