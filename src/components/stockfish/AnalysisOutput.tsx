...
function MoveStrip({
  ...
  return (
    <div className="flex min-w-0 flex-wrap gap-x-1 gap-y-0.5 text-text-secondary">
      {items.map((it) =>
        it.kind === "num" ? (
...
function VariationLine({
  ...
  return (
    ...
    >
      <summary className="flex cursor-pointer list-none items-start gap-2 rounded px-0.5 py-0.5 text-left marker:content-none [&::-webkit-details-marker]:hidden">
        <span className="mt-0.5 text-[9px] text-text-muted group-open:rotate-90">▸</span>
        <span className="min-w-[40px] font-semibold text-accent-teal-bright">{score}</span>
        <span className="min-w-5 text-text-muted">#{line.multipv}</span>
...
export default function AnalysisOutput({
  ...
  return (
    <div className="flex min-h-0 flex-col gap-1.5">
      ...
      <div className="flex max-h-72 min-h-[5.5rem] flex-col gap-1 overflow-y-auto rounded-lg border border-border-subtle bg-bg-deepest p-2 font-mono text-[11px]">
