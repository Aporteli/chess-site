...
export function BoardOverlay({ status }: BoardOverlayProps) {
  ...
  const isAlternative = status === "alternative";
  ...
  return (
    <div
      className={[
        ...
        isAlternative ? "shadow-[inset_0_0_0_3px_rgba(127,192,175,0.7)]" : "",
      ].join(" ")}
    >
      {(isCorrect || isMistake || isAlternative) && (
        <div
          className={[
            ...
            isCorrect
              ...
              : isAlternative
                ? "border-accent-teal/40 bg-accent-teal-dim/90 text-accent-teal-bright"
        ...
        >
          ...
          {isCorrect
            ? "Correct move!"
            : isAlternative
              ? "Correct — another book move remains"
              : "That's a mistake — try again"}
        </div>
      )}
    </div>
  );
